...
const Navbar = () => {
    ...
    return (
        <div
            className={
                `w-full fixed px-10 flex justify-between items-center transition-all duration-500 delay-100 z-[55] ${isTop ? "h-20 bg-black" : "h-14 bg-white"
                }`}
        >
            <div className={`flex items-center hover:text-gray-500 text-white cursor-pointer ${!isTop ? "pl-15" : ""}`} onClick={() => { router.push("/") }}>
                <Logo fill={isTop ? "#fff" : "#6b7280"} width={isTop ? "3.5em" : "2.5em"} height={isTop ? "3.5em" : "2.5em"} />
                {isTop && <p className="pl-4 font-bold text-2xl">Cleva Tech</p>}
            </div>
            <ul className="list-none flex gap-x-4 tracking-wider font-semibold">
                ...
                {navbarRoutes.map((item: NavbarRoutesType, index: number) => (
                    <li
                        key={index}
                        className={`relative inline-block cursor-pointer  ${!isTop ? "text-gray-500 hover:text-black" : "hover:text-gray-500 text-white"
                            } group`}
                        onClick={() => {
